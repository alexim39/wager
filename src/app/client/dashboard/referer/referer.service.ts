...
export interface RefererInterface {
    user: string;
    no: number;
    signUpDate: string;
    bonusAmount: number;
...
export class RefererService {
  ...
  getUserReferers(userId: string): Observable<ServerResponse> {
    this.showSpinner.next(true);
    return this.http.get<ServerResponse>(`${this.API_DOMAIN}/api/user/referers/${userId}`, httpOptions)
      .pipe(
